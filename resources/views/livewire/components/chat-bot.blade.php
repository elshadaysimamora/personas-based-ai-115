<div class="flex flex-col h-screen" x-data="{
    ...
    sendMessage() {
        ...
        $wire.sendMessage(this.text)
            ...
            .catch(error => {
                this.handleError(error.message || 'Gagal mengirim pesan.');
            });

        this.text = '';
    },

    // Memproses respons AI
    processAIResponse() {
        $wire.respond()
            .then(() => {
                this.sending = false;
            ...
            .catch(error => {
                this.handleError(error.message || 'Gagal memproses respons AI.');
            });
    },
    ...
    // Scroll ke bawah dengan pengecekan elemen
    scrollToBottom() {
    ...
    init() {
        ...
        // Event listener untuk stream update
        const streamHandler = () => this.scrollToBottom();
        window.addEventListener('stream-update', streamHandler);

        // Event listener untuk error AI
        ...
        this.$el.addEventListener('destroy', () => {
            window.removeEventListener('stream-update', streamHandler);
        });
    }
}" x-init="init()">
        ...
        <div class="max-w-3xl mx-auto p-4 space-y-6">
            ...
            @if ($conversation->messages->isEmpty())
                <div class="text-center text-gray-400 h-full flex flex-col justify-center items-center space-y-6 py-20">
                    ...
                    <p class="text-gray-500 max-w-md">Tanyakan apa saja untuk memulai percakapan. AI assistant akan membantu Anda</p>
                </div>
            @else
                <ul class="space-y-6">
                    <!-- Messages -->
                        ...
                        <li class="{{ $message->is_user_message ? 'flex justify-end' : 'flex justify-start' }}"
                            ...
                            @else
                                ...
                                <div class="flex max-w-3xl w-full">
                                    <div class="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center mr-3 flex-shrink-0 mt-1">
                    ...
                    <template x-if="sending">
                        <li class="flex justify-start">
                            <div class="flex max-w-3xl w-full">
                                ...
                                <div class="prose prose-invert text-gray-200 bg-gray-800 px-4 py-3 rounded-lg shadow-md chat-container w-full">
            ...
            <div class="flex items-center space-x-2 bg-gray-900 rounded-3xl p-3">
                ...
                <div class="flex-grow relative">
                    <textarea x-model="text"
                              @keydown.enter="if(!$event.shiftKey) sendMessage(); else $event.target.value += '\n'"
                              @input="debounceInput"
                              :disabled="sending"
                    ...
                    <span x-cloak x-show="sending">
                        <svg class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            ...
                            <path class="opacity-75" fill="currentColor"
                                  d="M4 12a8 8 0 018-8V2a10 10 0 00-10 10h2zm12-8v2a8 8 0 010 16h-2a10 10 0 0010-10h-2z"></path>
                        </svg>
                    </span>
                </button>
            </div>
